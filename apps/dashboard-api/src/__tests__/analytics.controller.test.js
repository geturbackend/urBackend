...
describe('Analytics Controller', () => {
    ...
    describe('getGlobalStats', () => {
        it('should return aggregated global stats with user counts fetched in parallel', async () => {
            ...
            mockGetConnection
                .mockResolvedValueOnce(mockDbConn1)
                .mockResolvedValueOnce(mockDbConn2);

            await controller.getGlobalStats(req, res, next);

            expect(mockGetConnection).toHaveBeenCalledTimes(2);
            expect(mockGetConnection).toHaveBeenNthCalledWith(1, '60c72b2f9b1d8b22fc5a87ba');
            expect(mockGetConnection).toHaveBeenNthCalledWith(2, '60c72b2f9b1d8b22fc5a87bb');
    ...
    describe('getActivationFunnel', () => {
        it('should return status of activation funnel steps', async () => {
            ...
            expect(steps[0]).toEqual({ step: 'signup_completed', order: 1, completed: true, completedAt: '2026-06-10T00:00:00Z' });
            expect(steps[1]).toEqual({ step: 'email_verified', order: 2, completed: true, completedAt: '2026-06-10T00:05:00Z' });
            expect(steps[2]).toEqual({ step: 'project_created', order: 3, completed: false, completedAt: null });
        });
    });
    ...
    describe('getRetention', () => {
        ...
        it('should return false flags if no signup event exists', async () => {
            ...
            expect(res.json).toHaveBeenCalledTimes(1);
            const responseData = res.json.mock.calls[0][0];
            expect(responseData.data).toEqual({ d1: false, d7: false, d30: false, signupDate: null });
        });
    });
