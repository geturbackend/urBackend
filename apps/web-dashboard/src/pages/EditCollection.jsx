...
// FUNCTION - EDIT COLLECTION COMPONENT
function EditCollection() {
    ...
    // Fetch existing collections for Ref picker and load the current collection schema
    useEffect(() => {
        let isMounted = true;
        const fetchCollections = async () => {
            ...
            try {
                ...
                if (isMounted) {
                    ...
                    const existingCollection = projectData.collections?.find(c => c.name.toLowerCase() === initialName);
                    if (existingCollection && existingCollection.model) {
                        // We need to ensure each field has an _id for the React keys
                        const mapFields = (fieldsArray) => {
                        ...
                        setFields(mapFields(existingCollection.model));
                    }
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await api.put(`/api/projects/${projectId}/collections/${collectionName}`, {
            ...
            toast.error(typeof errMsg === 'string' ? errMsg : JSON.stringify(errMsg) || "Failed to update collection");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="container" style={{ maxWidth: '900px' }}>
            ...
            <div className="card">
                <h2 style={{ marginBottom: '2rem', fontSize: '1.5rem', fontWeight: 600 }}>Edit Collection</h2>
