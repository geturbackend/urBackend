...
const UsageProgressBar = ({ label, used, limit, unit = 'MB', formatValue: customFormat, unlimited = false, tooltip = '' }) => {
  ...
  let statusClass = '';
  if (hasPercentage && percentage > 80) statusClass = 'danger';
  else if (hasPercentage && percentage > 60) statusClass = 'warning';
  ...
  return (
    <div style={{ marginBottom: '1rem', cursor: tooltip ? 'help' : 'default' }} title={tooltip}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: '4px' }}>
        ...
        <span style={{ color: 'var(--color-text-main)', fontWeight: 600 }}>{isUnlimited ? '∞' : (hasPercentage ? `${percentage}%` : '—')}</span>
      </div>
      {!isUnlimited && (
