...
You need a [urBackend account](https://app.urbackend.in) and a MongoDB connection string (Atlas free tier works). The base URL for all API calls is `https://api.urbackend.in`.

<Steps>
  <Step title="Create a project and get your API keys">
    Open the [urBackend Dashboard](https://app.urbackend.in) and create a new project. During setup, you will provide your MongoDB connection string.
  ...
  <Step title="Insert a document using your secret key">
    ...
    ```bash curl
    curl -X POST "https://api.urbackend.in/api/data/products" \
    ...
    ```javascript fetch
    const response = await fetch('https://api.urbackend.in/api/data/products', {
      method: 'POST',
      headers: {
        'x-api-key': 'sk_live_...',
  ...
  <Step title="Read documents using your publishable key">
    ...
    ```bash curl
    curl "https://api.urbackend.in/api/data/products" \
