...
**Base URL:** `https://api.urbackend.in`  
**Pattern:** `https://api.urbackend.in/api/data/:collectionName`
...
```javascript
const res = await fetch('https://api.urbackend.in/api/data/posts', {
  method: 'POST',
  headers: {
...
```bash
curl "https://api.urbackend.in/api/data/posts" \
...
```bash
curl "https://api.urbackend.in/api/data/posts/64fd1234abcd5678ef901234" \
  -H "x-api-key: pk_live_YOUR_KEY"
```
...
```bash
curl "https://api.urbackend.in/api/data/posts?status=published&price_gt=10&price_lt=50&sort=-createdAt" \
  -H "x-api-key: pk_live_YOUR_KEY"
```
...
const res = await fetch(`https://api.urbackend.in/api/data/posts/${postId}`, {
  method: 'PUT',
  headers: {
    'Content-Type': 'application/json',
    'x-api-key': 'sk_live_YOUR_KEY'
...
```bash
curl -X DELETE "https://api.urbackend.in/api/data/posts/64fd1234abcd5678ef901234" \
...
```bash curl
curl -X DELETE "https://api.urbackend.in/api/data/posts/64fd1234abcd5678ef901234?permanent=true" \
  -H "x-api-key: sk_live_YOUR_KEY"
```
