...
The AI Query Builder and the AI Collection Creator (Schema Design) each have their own monthly counter on the platform key:

| Plan | Personal Groq key allowed? | Query Builder requests per month | Collection Creator schema designs per month |
| :--- | :--- | :--- | :--- |
| Free | Yes | 5 | 1 |
| Pro | Yes | 20 | 5 |

Monthly counters reset on the first of each calendar month (UTC).

Collection Creator chat sessions that use the platform key are also capped at **20 iterations per session**. BYOK requests are not subject to this per-session cap.

Once your plan's platform-key cap (monthly or per-session) is reached, further AI requests return `403 Forbidden` until either the counter resets or you configure a BYOK key.
...
```bash
# Save a key
curl -X PUT "https://platform-api.urbackend.in/api/user/me/byok" \
...
```bash
curl -X PUT "https://platform-api.urbackend.in/api/projects/PROJECT_ID/byok" \
  -H "Content-Type: application/json" \
  -H "Cookie: <your-session-cookie>" \
  -d '{"groqKey": "gsk_..."}'
```
