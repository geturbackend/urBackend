...
1. Open your project in the [urBackend Dashboard](https://app.urbackend.in).
...
const loginResponse = await fetch('https://api.urbackend.in/api/userAuth/login', {
  method: 'POST',
  headers: {
    'x-api-key': 'pk_live_...',
...
const postResponse = await fetch('https://api.urbackend.in/api/data/posts', {
  method: 'POST',
  headers: {
    'x-api-key': 'pk_live_...',
...
// result will include the auto-injected owner field:
