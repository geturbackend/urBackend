...
|---|---|---|---|
| `to` | `string \| string[]` | Yes | Recipient email address(es). Array form must contain at least one entry. |
| `replyTo` | `string \| string[]` | No | Reply-to address(es). Forwarded to Resend as the `Reply-To` header. Array form must contain at least one entry. |
