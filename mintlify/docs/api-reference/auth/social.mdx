...
3. Copy the read-only **Callback URL** shown by urBackend (e.g., `https://api.urbackend.in/api/userAuth/social/github/callback`).
...
window.location.href =
  'https://api.urbackend.in/api/userAuth/social/github/start?x-api-key=pk_live_YOUR_KEY';

// Or for Google:
window.location.href =
  'https://api.urbackend.in/api/userAuth/social/google/start?x-api-key=pk_live_YOUR_KEY';
```
...
function AuthCallback() {
  ...
  useEffect(() => {
    async function handleCallback() {
      ...
      const res = await fetch('https://api.urbackend.in/api/userAuth/social/exchange', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
