...
<ParamField query="limit" type="number">
  Number of documents per page. Defaults to `100`, maximum `1000`.
</ParamField>
...
const res = await fetch(
  `https://api.urbackend.in/api/data/posts?${params}`,
  {
    headers: {
...
```bash curl
curl "https://api.urbackend.in/api/data/posts?page=1&limit=10&sort=-createdAt&status=published&include_deleted=true" \
  -H "x-api-key: pk_live_YOUR_KEY"
```

```javascript Private collection (RLS)
const res = await fetch('https://api.urbackend.in/api/data/notes', {
  headers: {
    'x-api-key': 'pk_live_YOUR_KEY',
