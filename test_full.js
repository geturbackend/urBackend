...
async function test() {
  await mongoose.connect('mongodb://127.0.0.1:27017/test_isdeleted2');
  ...
  const Model = mongoose.model('TestDoc2', schema);
  ...
  const reqQuery = {};
  const baseFilter = {}; // simulating req.rlsFilter
  ...
  features.sort().limitFields(); // not populate for now
  features.paginate();
